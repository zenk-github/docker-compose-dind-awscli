...
* The popular dind image has not insatalled `docker-compose`

The last point is because `docker:stable` is based on Alpine Linux and `awscli` does not work on Alpine distribution due to missing glibc libraries.
...
3. Downloads & installs `awscli` using the method described in https://docs.aws.amazon.com/cli/latest/userguide/install-cliv2-linux.html
4. Downloads & install `docker-compose`
